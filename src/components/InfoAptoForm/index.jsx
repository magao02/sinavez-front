...
import { AddBedButton, CamaInputContainer, Container, InputContainer,CheckBoxInputs } from "./styles";
...
const InfoAptoForm = () => {
  ...
  // BOTOES DE RADIO
  const addRadioInfo = (type, value) => {
      var key = type.toLowerCase();
      setRadioInput({...radioInput, [key]: value})
  }
    ...
    <Container>
      ...
      <InputContainer>
        ...
        <CamaInputContainer>
          {camas.map((id) => {
            return (
              <CamaInput id={id} deleteCama={deleteCama} handleCama={handleCama}/>
            )
          })}
            <AddBedButton onClick={addCamas}>+<span>ADICIONAR CAMA</span></AddBedButton>
        </CamaInputContainer>
        <Input
            variant="default"
            name="Endereço"
          ...
          />
          <CheckBoxInputs>
            <RadioInput title={"Tipo"} op1={"Comum"} op2={"PCD"}  addRadioInfo={addRadioInfo} />
            <RadioInput title={"Andar"} op1={"Terreo"} op2={"1`"} addRadioInfo={addRadioInfo} />
            <RadioInput title={"Suite"} addRadioInfo={addRadioInfo}/>
            <RadioInput title={"Wifi"} addRadioInfo={addRadioInfo} />
            <RadioInput title={"Animais"} border={"none"} addRadioInfo={addRadioInfo} />
          </CheckBoxInputs>
      </InputContainer>
    </Container>
  );
